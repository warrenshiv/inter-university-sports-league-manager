...
export async function transferICP(lenderAddress, amount, memo) {
  const canister = window.canister.ledger;
  const account = AccountIdentifier.fromHex(lenderAddress);
...
export async function balance() {
  const canister = window.canister.ledger;
  const address = await window.canister.loanManager.getAddressFromPrincipal(
